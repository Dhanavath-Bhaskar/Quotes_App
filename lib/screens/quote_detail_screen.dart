...
import 'package:qns/widgets/quote_full_screen_for_share.dart';
...
import '../services/google_translation_service.dart'; // <-- Import here

// --- CATEGORY-AUDIO & EMOJI MAPS as in your code above (not repeating for brevity) ---
...
class _QuoteDetailScreenState extends State<QuoteDetailScreen> {
  ...
  String _uiTitle = 'Quote Details';
  String _uiShareImage = 'Share Image';
  String _uiShareVideo = 'Share Video';
  String _uiRecord = 'Record';
  String _uiAddToFav = 'Add to Favorites';
  String _uiRemoveFromFav = 'Remove from Favorites';

  Stream<DocumentSnapshot>? _settingsStream;

  String get _quote => widget.quoteData['kQuote'] ?? widget.quoteData['quote'] ?? 'No quote found';
  String get _author => widget.quoteData['kAuthor'] ?? widget.quoteData['author'] ?? 'Unknown';
  String get _category => widget.quoteData['kCategory'] ?? widget.quoteData['category'] ?? 'Uncategorized';
  String get _imageUrl => widget.imageUrl.isNotEmpty
      ? widget.imageUrl
      : (widget.quoteData['imageUrl'] ?? '');
  String get _docId {
    if (widget.docId != null && widget.docId!.isNotEmpty) return widget.docId!;
    final base = '${_quote}|${_author}|${_category}';
  ...
  void initState() {
    ...
    if (_userId.isNotEmpty) {
      ...
      _settingsStream!.listen((doc) async {
        String lang = 'en';
        if (doc.exists && doc.data() != null) {
          lang = doc['preferredLanguage'] ?? 'en';
        }
        await _updateLanguageAndTranslateUI(lang);
      });
    } else {
      _updateLanguageAndTranslateUI('en');
    }
  }

  Future<void> _checkFavorite() async {
    if (_userId.isEmpty || _docId.isEmpty) {
      setState(() {
        _isFavorite = false;
    ...
    final doc = await _favCol.doc(_docId).get();
    setState(() {
  ...
  Future<void> _toggleFavorite() async {
    if (_loadingFavorite || _userId.isEmpty) return;
    setState(() => _loadingFavorite = true);
    final favDoc = _favCol.doc(_docId);
    if (_isFavorite) {
      await favDoc.delete();
  ...
  Future<void> _updateLanguageAndTranslateUI(String lang) async {
    if (lang == _preferredLanguage && _uiTitle != 'Quote Details') return;
    setState(() {
      _translating = true;
    ...
    if (lang == 'en') {
      setState(() {
        _uiTitle = 'Quote Details';
        _uiShareImage = 'Share Image';
        _uiShareVideo = 'Share Video';
        _uiRecord = 'Record';
        _uiAddToFav = 'Add to Favorites';
        _uiRemoveFromFav = 'Remove from Favorites';
        _ui['quote'] = _quote;
        _ui['author'] = _author;
        _ui['category'] = _category;
      });
    } else {
      try {
        final results = await Future.wait([
          _googleTranslator.translate(text: 'Quote Details', targetLang: lang),
          ...
          _googleTranslator.translate(text: 'Add to Favorites', targetLang: lang),
          _googleTranslator.translate(text: 'Remove from Favorites', targetLang: lang),
          _googleTranslator.translate(text: _quote, targetLang: lang),
          _googleTranslator.translate(text: _author, targetLang: lang),
          _googleTranslator.translate(text: _category, targetLang: lang),
        ]);
        setState(() {
          _uiTitle = results[0];
          ...
          _uiRecord = results[3];
          _uiAddToFav = results[4];
          _uiRemoveFromFav = results[5];
          _ui['quote'] = results[6];
          _ui['author'] = results[7];
          _ui['category'] = results[8];
        });
      } catch (e) {
  ...
  Future<void> _shareAsImage() async {
    final emoji = (_categoryEmoji[_category] ?? '');
    _pendingShareData = {
      'quote': _ui['quote'],
      'author': _ui['author'],
      'imageUrl': _imageUrl,
  ...
  Future<void> _shareAsVideo() async {
    final emoji = (_categoryEmoji[_category] ?? '');
    final audioAsset = _categoryAudio[_category] ?? _categoryAudio['All']!;
    _pendingShareData = {
      'quote': _ui['quote'],
      'author': _ui['author'],
      'imageUrl': _imageUrl,
    ...
    final tmp = await getTemporaryDirectory();
    final fileName = 'quote_share_card_${DateTime.now().millisecondsSinceEpoch}.png';
    ...
    try {
      ...
      final videoFileName = 'quote_share_video_${DateTime.now().millisecondsSinceEpoch}.mp4';
  ...
  Widget build(BuildContext context) {
    final emoji = (_categoryEmoji[_category] ?? '');
    final int emojiSeed = _docId.hashCode;
                ...
                appBar: AppBar(
                  ...
                  actions: [
                    IconButton(
                      ...
                      tooltip: _isFavorite ? _uiRemoveFromFav : _uiAddToFav,
